...
Prepare an SD-card with Raspberry Pi OS. I have used the version from November 19th 2024 (64-bit) with desktop but without recommended software. My system uses a Raspberry Pi 4B. The advantage of desktop support is that a standalone system can be created by attaching an external monitor, mouse and keyboard. This creates the possibility of implementing a very small and modular control system.\\
...
\subsection{Install BCM2835 v1.75}
Install BCM2835-1.75 so that the pin IO on the Raspberry Pi can be used:
%% setup bmc
\begin{verbatim}
...
If the use of RDP is required. Install the package using the following commands.
...
If the RDP connection cannot be established using the admin account, a new user account must be made. 

\begin{verbatim}
	sudo adduser rdpuser
\end{verbatim}

Provide a password. All the other information is not required.
